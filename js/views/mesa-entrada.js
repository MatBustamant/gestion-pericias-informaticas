...
window.setMF = function(f) { S.mesaEstado=f; init_mesa_entrada(); };
...
window.init_mesa_entrada = function() {
  ...
  document.getElementById('mesa-filters').innerHTML = fl.map(f=>'<button class="filter-btn'+(S.mesaEstado===f?' active':'')+'" onclick="setMF(\''+f+'\')">'+flbl[f]+'</button>').join('');
